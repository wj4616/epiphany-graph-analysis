# Verification Gates V1–V7 — Operational Reference (N8)

## Usage

N8 runs this battery in order after constructing the primary conclusion. Each gate is applied as an explicit separate check. All gates must be applied; none may be skipped. Pass/fail/partial recorded for each.
...
2. Flag any stage whose output is not cited or used anywhere in the N8 conclusion or verification process
...
**Note:** V4 is the only gate that can trigger an external action (back-edge retry via SKILL.md STEP 3c). All other gates are final within N8.
...
**CRITICAL:** Write `stages/N8-v6-scope.txt` with exactly these 3 labels and fill each in:
...
**Output:** Write file `stages/N8-v6-scope.txt`. Also include V6 entry in N8 verification report:
`V6 Scope: [PASS] — wrote stages/N8-v6-scope.txt`
...
## V1–V7 Summary Block (required at end of N8 output)
